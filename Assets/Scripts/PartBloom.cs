...
public class PartBloom : MonoBehaviour
{
    ...
    /// Bloom权值
    /// </summary>
    [Range(0.0f, 1f)] public float bloomFactor = 0.5f;
    ...
    private void OnPostRender()
    {
        ...
        if (UseBloom)
        {
            if (BloomMat)
            {
                ...
                RenderTexture ThresholdMaskTex = RenderTexture.GetTemporary(cameraRenderTex.width,
                    cameraRenderTex.height, 0, RenderTextureFormat.Default);
                BloomMat.SetVector(_colorThreshold, colorThreshold);
                Graphics.Blit(cameraRenderTex, ThresholdMaskTex, BloomMat, 0);

                RenderTexture AddBlurt = RenderTexture.GetTemporary((int) cameraRenderTex.width / 4,
                    (int) cameraRenderTex.height / 4, 0, RenderTextureFormat.ARGBHalf);
                ...
                for (int i = 0; i < blurNum; i++)
                {
                    ...
                    RenderTexture blurTemp = RenderTexture.GetTemporary((int) cameraRenderTex.width / BlurPart,
                        (int) cameraRenderTex.height / BlurPart, 0, RenderTextureFormat.ARGBHalf);
                    ...
                    else
                    {
                        ...
                        RenderTexture AddBlurtTemp = RenderTexture.GetTemporary((int) cameraRenderTex.width / 4,
                            (int) cameraRenderTex.height / 4, 0, RenderTextureFormat.Default);
